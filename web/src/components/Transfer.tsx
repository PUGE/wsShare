        ...
        <div className="actions">
          <div className="buttons">
              ...
              <>
                ...
                {transfer.fileType === 'text/plain' ? (
                  <CopyToClipboard text={text} onCopy={onCopy}>
                    <button>{copied ? '已复制' : '发送到粘贴板'}</button>
                  </CopyToClipboard>
                ) : null}
              </>
            ) : null}
            ...
            transfer.state === TransferState.FAILED ? (
              <button onClick={dismissTransfer}>清除记录</button>
            ) : null}
            {transfer.state === TransferState.INCOMING ? (
              <>
                <button onClick={acceptTransfer}>接收</button>
                <button onClick={rejectTransfer}>拒绝</button>
              </>
            ) : null}
            {cancellableStates.includes(transfer.state) ? (
              <button onClick={cancelTransfer}>取消</button>
            ) : null}
          </div>
          <AnimatePresence>
